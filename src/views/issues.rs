...
impl IssuesView {
    ...
    pub fn set_loaded(&mut self, issues: Vec<IssueInfo>) {
        self.issues = issues;
        self.loading_state = LoadingState::Loaded(());
        if !self.issues.is_empty() && self.selected >= self.issues.len() {
    ...
    pub fn update(&mut self, issues: Vec<IssueInfo>) {
        self.set_loaded(issues);
    }

    pub fn selected_issue(&self) -> Option<&IssueInfo> {
        ...
        let matches: Vec<usize> = self
            ...
            .filter(|(_, issue)| {
                issue.title.to_lowercase().contains(&query_lower)
                    || issue.author.login.to_lowercase().contains(&query_lower)
                    || issue
                        .labels
                        .iter()
    ...
    pub fn render(&mut self, area: Rect, buf: &mut Buffer, theme: &Theme, focused: bool) {
        ...
        let title = match &self.loading_state {
            LoadingState::Loading { spinner_frame, .. } => {
                format!(" Issues {} ", SPINNER_FRAMES[*spinner_frame])
            }
            LoadingState::Timeout => " Issues (timeout) ".to_string(),
            LoadingState::Error(_) => " Issues (error) ".to_string(),
            _ => format!(" Issues ({}) ", self.issues.len()),
        };
        ...
        match &self.loading_state {
            ...
            LoadingState::Loading { spinner_frame, .. } => {
                ...
                let msg = format!("{} Loading issues...", spinner);
                let x = inner.x + (inner.width.saturating_sub(msg.len() as u16)) / 2;
                let y = inner.y + inner.height / 2;
        ...
        self.max_content_width = self
            ...
            .iter()
            .map(|issue| {
                let number_width = format!("#{}", issue.number).len();
                let state_width = format!("[{}] ", issue.state).len();
                let title_width = issue.title.chars().count();
                let author_width = format!(" ({})", issue.author.login).len();
                let labels_width = if !issue.labels.is_empty() {
                    issue.labels.iter().map(|l| l.name.len() + 2).sum::<usize>() + 1
                } else {
                    0
                };
                number_width + state_width + title_width + author_width + labels_width + 2
            })
            .max()
            .unwrap_or(0)
            + 2;
            ...
            .enumerate()
        {
            ...
            let line = format!(
                "#{} [{}] {} ({}){}",
                issue.number, issue.state, issue.title, issue.author.login, labels_str
            );
        ...
        let scrollbar = Scrollbar::new(self.issues.len(), height, self.offset);
